/* ======================================================================
  File Name     : Stack.C
  Description   : stack is implemented using SLL with variable data type in a stack node.
...
/* stack data has sample's tree node's addr */
#define TREE_NODE_MSG          3
 /* stack's data has Employee's record */
#define EMP_MSG                1
/* tree node's data has sample alpha data */
#define NUM_MSG                2
#define ALPHA_DATALEN          (sizeof(char))
/* max data size does queue & tree can have */
#define MAX_TREE_DATA         20
#define QUIT_OPER              1
#define PUSH_DATA_OPER         2
#define POP_DATA_OPER          3
/* destroy stack */
#define DESTROY_STACK_OPER     4
...
#define STACK_MEGA_HEAD        3
  /* display stack node's data */
#define RETRIEVE_NODE          (DISPLAY_OPER + 1)
/* maximum no of stack nodes */
#define MAX_STACK_SIZE        20
#define OPER_SIZE             15
#define MAX_NAME              (MAX_TREE_DATA - sizeof(unsigned) - sizeof(unsigned char))
/* does not allow access to queue */
#define NO_ACCESS              1
/* allow access to queue */
#define FREE_ACCESS            0
 /* invalid data that tree node can have */
#define INVALID_DATA         255
/* enable global to trace execution flow for debugging */
#define TRACE_ON               1
/* disable global to trace execution flow for debugging */
#define TRACE_OFF              0
#define INPUT_STR_MAX_CHARS   (7)
#define NULL_DATA_PTR         ((void *)0)
#define NULL_CHAR             '\0'

#define EMP_MIN_ID                      (1)
#define EMP_MAX_ID                      (1000)
#define MSG_ID_NUM_MIN_VAL              (1)
#define MSG_ID_NUM_MAX_VAL              (100)
...
char input_str[INPUT_STR_MAX_CHARS];
...
uint16_t Get_Validate_Input_Number(void *const int32_input_num_ptr, char *const input_str_ptr, const unsigned int input_INPUT_STR_MAX_CHARS, const int32_t valid_min_value, const int32_t valid_max_value);
uint16_t Get_Input_Str(char *const input_str_ptr, const unsigned int input_INPUT_STR_MAX_CHARS);
...
uint16_t Get_Input_Alpha_Char_Str(char *const input_str_ptr, const unsigned int input_INPUT_STR_MAX_CHARS);
...
int main()
{
    ...
    do
    {
       printf("\n MENU: 1:Quit, 2:Push, 3:Pop, 4:Destroy Stack, 5:Display: Enter: ");
      // scanf("%d", &nav_mode);
	   if((Get_Validate_Input_Number(&nav_mode, input_str, INPUT_STR_MAX_CHARS, QUIT_OPER, DISPLAY_OPER)) != SUCCESS)
	   {
		   nav_mode = DISPLAY_OPER + 1;
	   }
       switch(nav_mode)
       {
		   case PUSH_DATA_OPER:
		   ...
		   case QUIT_OPER:
		     ...
		     break;
		   default:
		     printf("\n ERR[1.3]: Invalid stack oper: %d", nav_mode);
       }
...
*******************************************************************/
 int Display_Data(void)
 {
     ...
     printf("\n Display Stack  - 1: Top,  2: Whole,  3: Meta Data ");
     printf("\n Enter Display Mode: ");
   // scanf("%d", &display_mode);
     if((Get_Validate_Input_Number(&display_mode, input_str, INPUT_STR_MAX_CHARS, RETRIEVE_TOP, STACK_MEGA_HEAD)) != SUCCESS)
     {
		  display_mode = STACK_MEGA_HEAD + 1;
	 }
     switch(display_mode)
     {
...
int Push_EmpData(stack_head *stack_megaptr, stack_data_t *push_emp_ptr)
{
	...
	if(push_emp_ptr == NULL)
	{
       ...
       /* name's len also includes NUL character:'\0' */
	    if((Get_Validate_Input_Number(&emp_id, input_str, INPUT_STR_MAX_CHARS, EMP_MIN_ID, EMP_MAX_ID)) != SUCCESS)
		{
			printf("\n ERR: Invalid Emp ID ");
		    return ERR_INVALID_DATA;
		}
		printf("\n Enter Emp Name : ");
	    if((Get_Input_Alpha_Char_Str(name, MAX_NAME)) != SUCCESS)
		{
...
int Push_Data_Msg(stack_head *stack_megaptr)
{
	...
	printf("\n Push Data Type: 1 - Emp, 2 - a number ");
	printf("\n Enter : ");
	//scanf("%d", &msgtype);
	if((Get_Validate_Input_Number(&msgtype, input_str, INPUT_STR_MAX_CHARS, EMP_MSG, NUM_MSG )) != SUCCESS)
	{
	    msgtype	= NUM_MSG + 1;
    }
	switch(msgtype)
...
int Push_NumData(stack_head *stack_megaptr, stack_data_t *push_num_ptr)
{
	...
	if(push_num_ptr == NULL)
	{
	    printf("\n Enter a num : ");
      //  scanf("%d", &num);
	   if((Get_Validate_Input_Number(&num, input_str, INPUT_STR_MAX_CHARS, MSG_ID_NUM_MIN_VAL, MSG_ID_NUM_MAX_VAL)) != SUCCESS)
       {
          printf("\n ERR[25.1]: Invalid Msg value");
	      return FALSE;
       }
       if((stack_data_ptr = (stack_data_t *)calloc (1, sizeof(stack_data_t))) == NULL)
	   {
...
BUGS           :
-*------------------------------------------------------------*/
uint16_t Get_Validate_Input_Number(void *const input_num_ptr, char *const input_str_ptr, const unsigned int input_INPUT_STR_MAX_CHARS, const int32_t valid_min_value, const int32_t valid_max_value)
{
	...
	*int32_input_num_ptr = 0;
	if((Get_Input_Str(input_str_ptr, input_INPUT_STR_MAX_CHARS)) != SUCCESS)
		return FAILURE;
	if((Str_to_Num_Conv(&temp_int, input_str_ptr)) != SUCCESS)
	{
		memset(input_str_ptr, NULL_CHAR, input_INPUT_STR_MAX_CHARS);
		return FAILURE;
	}
	memset(input_str_ptr, NULL_CHAR, input_INPUT_STR_MAX_CHARS);
...
uint16_t Get_Input_Str(char *const input_str_ptr, const unsigned int input_INPUT_STR_MAX_CHARS)
{
    ...
    if(input_str_ptr == NULL_DATA_PTR || input_INPUT_STR_MAX_CHARS <= 1)
	{
		return FAILURE;
	}
    memset(input_str_ptr, NULL_CHAR, input_INPUT_STR_MAX_CHARS);
	while (1)
	{
		...
		switch(rcvd_char)
        {
            ...
            break;
            default:
     			if(input_str_num_chars + 1 < input_INPUT_STR_MAX_CHARS )
    		    {
