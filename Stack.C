...
int Initialize_Stack(stack_head *stack_megaptr);
    ...
    do
    {
       ...
       switch(nav_mode)
       {
		   ...
		   case DESTROY_STACK_OPER:
		     if(StackData_Range(nav_mode, &stack_megahead) != TRUE)
		       continue;
		   break;
           case DISPLAY_OPER:
		     Display_Data();
   ...
   if((Access_Stack(stack_megaptr)) == FALSE)
    ...
    switch(ret_mode)
    {
	   ...
	   case RETRIEVE_WHOLE_STACK:
	      ...
	      {
		    printf("\n ERR[17.7]: S's Head Ptr: %#X, is already empty to destroy",stack_megaptr);
			return FALSE;
	      }
 	      /* not a empty stack  */
 	      if((Access_Stack(stack_megaptr)) == FALSE)
		  ...
		  Control_Stack(stack_megaptr,FREE_ACCESS);
		  Initialize_Stack(stack_megaptr);
          ret_state = TRUE;
	   break;
       default:
         printf("\n ERR[17.6]: Invalid S's head addr: %#X, retrieve mode: %d", stack_megaptr, ret_mode );
    }
    Control_Stack(stack_megaptr, FREE_ACCESS);
	return ret_state;
 }
 ...
 int StackData_Range(unsigned mode, stack_head *stack_megaptr)
 {
	 ...
	 switch(mode)
     {
		 case DESTROY_STACK_OPER:
            ...
            printf("\n INFO[23.2]: Before Destroy:: S Head's Addr: %#X, current num of nodes: %u", stack_megaptr->stack_array, node_count);
